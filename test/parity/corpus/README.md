...
| `repos/managed` | **One file option overridden four times, scoped by path.** The order is meaning: a file matched by several entries takes the last of them, not the most specific. `parity/order/v1` is declared ahead of the broader `parity/order` on purpose, so a reading that preferred the most specific selector disagrees here and only here. |
