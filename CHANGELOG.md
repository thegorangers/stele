...
- **The ordinary supply-chain paperwork, added rather than left implicit.**
  [SECURITY.md](SECURITY.md) states how to report a vulnerability privately
  and what a single-maintainer project can honestly promise in response.
  [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) adopts the Contributor Covenant
  and reports through the same channel. CI gained a `govulncheck` job that
  reads the module graph and fails the build on a reachable known
  vulnerability, run separately from the hermetic build-and-test job because
  it needs the network to fetch the vulnerability database. `Dependabot` now
  opens weekly pull requests for Go modules and pinned GitHub Actions —
  raising them, never merging them, since this repository pins its
  dependencies on purpose. Each release gains a CycloneDX SBOM per binary,
  generated by `syft` through goreleaser's `sboms:` block, named in
  `SHA256SUMS` beside the binaries it describes and covered by the same
  cosign signature. None of this changes a byte `generate` or `export`
  writes.
