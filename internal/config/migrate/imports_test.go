...
// TestFloatingRefIsWarnedAbout is defect three. A ref that is not a commit is
// a label somebody else can move onto other content, and a third party's
// options land in the descriptor and therefore in the generated bytes. It is
// not a refusal — the manifest accepts a branch, and `stele` pins it in
// stele.lock — but it must not be silent.
func TestFloatingRefIsWarnedAbout(t *testing.T) {
	r := mustMigrateFS(t, map[string]string{
		"buf.yaml":     bufYAMLVendored,
		"buf.gen.yaml": genYAMLVendored,
		"Makefile": pinnedPlugin + "vendor:\n" +
			"\t@buf export \"ssh://git@git.example.com/group/catalog.git#subdir=api,ref=main\" --exclude-imports --output=third_party/proto\n",
		"api/example/orders/v1/orders.proto": `syntax = "proto3";
package example.orders.v1;
import "example/catalog/events/v1/events.proto";
`,
	...
	if !r.Complete() {
		t.Fatalf("unresolved = %#v, want none: a branch ref is a manifest this tool loads", r.Unresolved)
	}
	joined := strings.Join(r.Warnings, "\n")
	if !strings.Contains(joined, "main") || !strings.Contains(joined, "catalog") {
		t.Fatalf("warnings = %#v, want the floating ref named", r.Warnings)
	}
	out, err := r.YAML()
	if err != nil {
		t.Fatal(err)
