...
type Result struct {
	...
	// completion.
	Unresolved []string
	// Warnings name what translated faithfully but should not go unread.
	// They do not block completion: what they describe is a manifest this
	// tool loads and builds from, and refusing it here would refuse a shape
	// the rest of the tool accepts.
	Warnings []string
...
func (m *migration) warn(format string, a ...any) {
	m.result.Warnings = append(m.result.Warnings, fmt.Sprintf(format, a...))
}
...
func (m *migration) build() error {
	...
	for _, name := range m.depOrder {
		...
		case !isCommit(d.Ref):
			m.warn("deps %s: ref %s is not a full commit SHA, so it names a label somebody else can move onto other content — or is an abbreviation of a commit, which identifies content only while it stays unambiguous. A producer's options land in the descriptor and therefore in the generated bytes, so an upstream merge can change this repository's output with no change here. stele.lock records the commit each run resolved to, so a build stays reproducible until the next `--update`; pin a commit here if that is not enough.", name, d.Ref)
		}
