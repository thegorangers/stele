<picture>
  ...
  <img alt="stele" src="brand/logo-light.svg" width="44" align="left" hspace="14" vspace="4">
</picture>

# The stele guide

The complete reference: every command, every manifest field, the reasoning
behind each rule, and the boundaries of what has actually been measured. If you
have not used the tool before, start at the [README](../README.md) — it covers
installing, a first run, and migrating off buf.
...
- **This is not AIP coverage.** Five `aip/` rules across three AIPs, out of 29
  ...
  AIP, is [docs/AIP.md](AIP.md); a tool claiming AIP coverage without
...
- **A rename is declared, not inferred.** `breaking.moves` (shipped in v0.5.0)
  maps a proto package or file to its new name, and the previous revision is
  rewritten through that map before the comparison runs — so a lossless rename
  produces no findings, while a rename that also dropped a field still reports
  that field under its new name. Nothing infers the move for you: an
  undeclared rename still reads as a removal plus an addition.
...
[docs/ROADMAP.md](ROADMAP.md), per rule.
